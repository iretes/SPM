...
int main(int argc, char* argv[]) {
	...
	for(long i=0;i<nkeys; ++i) {
		for(long j=0;j<nkeys; ++j) {
			...
			if (map[i]>0 && map[j]>0) {
				#ifdef DEBUG
					std::printf("second,%ld,%ld,%ld\n", i*nkeys+j, map[i], map[j]);
				#endif
	...
	std::printf("%d,%ld,%ld,%d,%lf\n",
			1, nkeys, length, -1, time_elapsed);
}
